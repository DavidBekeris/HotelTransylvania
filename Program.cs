...
namespace HotelTransylvania
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            bool invalidInput = false; // TODO: Only accept certain keys - DONE
            ...
            while (insideMenu)
            {
                ...
                do
                {
                    ...
                    invalidInput = !(
                            ...
                            || menuChoice.Key == ConsoleKey.D4
                            || menuChoice.Key == ConsoleKey.D5
                ...
                switch (menuChoice.Key)
                {
                    ...
                    case ConsoleKey.D4: // Make reservation
                        ...
                        Room reservation = new Room(); // TODO: Making a reservation doesnt work, creates new ID number atm.
                            ...
                            for (int i = 0; i <= roomsList.Count; i++)
                            {
                                try
                                {
                                    if (reservation.roomId == roomsList[i].roomId)
                                    {
                                        ...
                                        reservation.customerFullName = Console.ReadLine();

                                        Console.Write("Enter date reservation starts for this id {0} (yyyy-mm-dd tt:hh): ", reservation.roomId);
                                        reservation.roomStartDate = DateTime.Parse(Console.ReadLine());
                                        //roomsList[counter++] = reservation;
                                        //roomsList.Add(reservation);

                                        Console.Write("Enter date reservation ends for this id {0} (yyyy-mm-dd tt:hh): ", reservation.roomId);
                                ...
                                {
                                    Console.WriteLine("No room with that ID. Nothing have been booked.");
                                    Thread.Sleep(3000);
                                }
                            }
                            break;
        ...
        private static void ListRooms(List<Room> roomsList)
        {
            ...
            foreach (Room room in roomsList)
            {
                ...
                Console.WriteLine("{0,-5} {1, -15}", room.roomId, room.roomDescription); // TODO: Check so output looks okey - DONE
            }
        }
